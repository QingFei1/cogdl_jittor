...
def read_geom_data(folder, dataset_name):
    ...
    for split in range(10):
        ...
        with np.load(graph_split_file_path) as splits_file:
            ...
            train_mask = jittor.array(train_mask).bool()
            val_mask = jittor.array(val_mask).bool()
            test_mask = jittor.array(test_mask).bool()
            all_masks.append({"train": train_mask, "val": val_mask, "test": test_mask})

    features = jittor.array(features,dtype=jittor.float)
    labels = jittor.array(labels,dtype=jittor.int64)

    coo_adj = adj.tocoo()
    row, col = jittor.array(coo_adj.row,dtype=jittor.int64), jittor.array(coo_adj.col,dtype=jittor.int64)
    edge_index = (row, col)
...
class GeomDataset(Dataset):
    ...
    def num_classes(self):
        ...
        return int(jittor.max(self.data.y)) + 1
...
def read_planetoid_data(folder, dataset_str):
    ...
    features = jittor.array(features.todense(),dtype=jittor.float)
