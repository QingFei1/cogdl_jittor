...
from cogdl_jittor.data import Graph, Dataset
from cogdl_jittor.utils import download_url
...
class GCCDataset(Dataset):
    ...
    def preprocess(self, root, name):
        ...
        edge_list = jittor.array(edge_list,dtype=jittor.int64).t()
...
class Edgelist(Dataset):
    ...
    def process(self):
        ...
        y = jittor.zeros(num_nodes, len(label2id))
        y[nodes, labels] = 1

        data = Graph(edge_index=jittor.array(edge_list,dtype=jittor.int64).t(), x=None, y=y)
