...
from cogdl_jittor.utils import  download_url, untar
from cogdl_jittor.utils.graph_utils import coalesce,remove_self_loops
import jittor
...
def read_planetoid_data(folder, prefix):
    ...
    # getA() matrix转化numpy
    x, tx, allx = tuple([jittor.array(item.todense().getA()).float() for item in [x, tx, allx]])
    y, ty, ally = tuple([jittor.array(item).float() for item in [y, ty, ally]])

    train_index = jittor.arange(y.size(0)).long()
    val_index = jittor.arange(y.size(0), y.size(0) + 500).long()
    ...
    if prefix.lower() == "citeseer":
        ...
        tx_ext = jittor.zeros(len_test_indices, tx.size(1))
        tx_ext[test_index_reorder - test_index.min(), :] = tx
        ty_ext = jittor.zeros(len_test_indices, ty.size(1))
        ty_ext[test_index_reorder - test_index.min(), :] = ty

        tx, ty = tx_ext, ty_ext

    x = jittor.concat([allx, tx], dim=0).float()
    y = jittor.concat([ally, ty], dim=0)
    y=jittor.arg_reduce(y,'max',dim=1,keepdims=False)[0].long()
    #y = jittor.concat([ally, ty], dim=0).max(dim=1)[1].long()
...
class Planetoid(Dataset):
    ...
    def __init__(self, root, name, split="public", num_train_per_class=20, num_val=500, num_test=1000):
        ...
        self.data = jittor.load(self.processed_paths[0])
        self.split = split
    ...
    def num_classes(self):
        ...
        return int(jittor.max(self.data.y).item()) + 1
    ...
    def process(self):
        ...
        jittor.save(data, self.processed_paths[0])
...
def normalize_feature(data):
    x_sum = jittor.sum(data.x, dim=1)
    x_rev = x_sum.pow(-1).flatten() 
    x_rev[jittor.misc.isnan(x_rev)] = 0.0 
    x_rev[x_rev==float('inf')] = 0.0
