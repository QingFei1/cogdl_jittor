...
from cogdl_jittor.data import Dataset, Graph, MultiGraphDataset
from cogdl_jittor.utils import Accuracy, MultiLabelMicroF1, MultiClassMicroF1, CrossEntropyLoss, BCEWithLogitsLoss
...
def scale_feats(data):
    ...
    feats = data.x.numpy()
    scaler.fit(feats)
    feats = jittor.array(scaler.transform(feats)).float()
    data.x = feats
    return data
...
def generate_random_graph(num_nodes=100, num_edges=1000, num_feats=64):
    ...
    edge_index = jittor.randint(0, num_nodes, (2, num_edges))
    x = jittor.randn(num_nodes, num_feats)
    y = jittor.randint(0, 2, (num_nodes,))

    # set train/val/test mask in node_classification task
    train_mask = jittor.zeros(num_nodes).bool()
    train_mask[0 : int(0.3 * num_nodes)] = True
    val_mask = jittor.zeros(num_nodes).bool()
    val_mask[int(0.3 * num_nodes) : int(0.7 * num_nodes)] = True
    test_mask = jittor.zeros(num_nodes).bool()
...
class GraphDataset(MultiGraphDataset):
    def __init__(self, path="cus_graph_data.pt", metric="accuracy"):
        ...
        # try:
        data = jittor.load(path)
        if hasattr(data[0], "y") and data[0].y is None:
            self.y = jittor.concat([idata.y for idata in data])
        self.data = data
