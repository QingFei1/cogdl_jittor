...
def read_geom_data(folder, dataset_name):
    ...
    features = BF.tensor(features, dtype=BF.dtype_dict["float32"])
    labels = BF.tensor(labels, dtype=BF.dtype_dict["int64"])

    coo_adj = adj.tocoo()
    row, col = BF.tensor(coo_adj.row, dtype=BF.dtype_dict["long"]), BF.tensor(coo_adj.col, dtype=BF.dtype_dict["long"])
    edge_index = (row, col)
...
def read_planetoid_data(folder, dataset_str):
    ...
    features = BF.tensor(features.todense(), BF.dtype_dict["float"])

    labels = np.argmax(labels, axis=-1)
    labels = BF.tensor(labels, dtype=BF.dtype_dict["long"])
