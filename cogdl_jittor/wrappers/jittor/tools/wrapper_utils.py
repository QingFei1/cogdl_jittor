...
from cogdl_jittor.utils import accuracy, multilabel_f1
...
def pre_evaluation_index(y_pred, y_true, sigmoid=False):
    ...
    if len(y_true.shape) == 1:
        ...
        tp = pred.sum()
        fnp = pred.shape[0] - tp
        return jittor.array((tp, fnp)).float()
    else:
        if sigmoid:
            border = 0.5
        else:
        ...
        tp = (y_pred * y_true).sum().astype(jittor.float32)
        # tn = ((1 - y_true) * (1 - y_pred)).sum().to(torch.float32)
        fp = ((1 - y_true) * y_pred).sum().astype(jittor.float32)
        fn = (y_true * (1 - y_pred)).sum().astype(jittor.float32)
        return jittor.array((tp, fp, fn))
...
def node_degree_as_feature(data):
    ...
    for i in range(len(data)):
        one_hot = nn.one_hot(degrees[i], max_degree).float()
        data[i].x = one_hot
    return data
...
class LogRegTrainer(object):
    def train(self, data, labels, idx_train, idx_test, loss_fn=None, evaluator=None, run=20):
        ...
        for _ in range(run):
            ...
            optimizer = jittor.optim.Adam(log.parameters(), lr=0.01, weight_decay=0.0)
