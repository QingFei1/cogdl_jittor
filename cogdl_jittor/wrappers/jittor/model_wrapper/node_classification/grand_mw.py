...
class GrandModelWrapper(NodeClfModelWrapper):
    ...
    def train_step(self, batch):
        ...
        if len(graph.y.shape) > 1:
            output_list = [jittor.sigmoid(x) for x in output_list]
        else:
            output_list = [nn.log_softmax(x, dim=-1) for x in output_list]
    ...
    def consistency_loss(self, logps, train_mask):
        ...
        ps = [jittor.exp(p)[jittor.logical_not(train_mask)] for p in logps]
        sum_p = 0.0
        for p in ps:
            sum_p = sum_p + p
        avg_p = sum_p / len(ps)
        sharp_p = (jittor.pow(avg_p, 1.0 / temp) / jittor.sum(jittor.pow(avg_p, 1.0 / temp), dim=1, keepdims=True)).detach()
        loss = 0.0
        for p in ps:
            loss += jittor.mean((p - sharp_p).pow(2).sum(1))
        loss = loss / len(ps)
