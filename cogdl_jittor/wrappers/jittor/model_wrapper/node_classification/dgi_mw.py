...
from ...tools.wrapper_utils import evaluate_node_embeddings_using_logreg
...
class DGIModelWrapper(UnsupervisedModelWrapper):
    ...
    def train_step(self, subgraph):
        ...
        num_nodes = x.shape[0]
        labels = jittor.zeros((num_nodes * 2,))
        labels[:num_nodes] = 1
    ...
    def setup_optimizer(self):
        ...
        return jittor.optim.Adam(self.parameters(), lr=cfg["lr"], weight_decay=cfg["weight_decay"])
...
class Discriminator(nn.Module):
    ...
    def execute(self, c, h_pl, h_mi, s_bias1=None, s_bias2=None):
        c_x = jittor.unsqueeze(c, 0)
        c_x = c_x.expand_as(h_pl)

        sc_1 = jittor.squeeze(self.f_k(h_pl, c_x), 1)
        sc_2 = jittor.squeeze(self.f_k(h_mi, c_x), 1)
        ...
        logits = jittor.concat((sc_1, sc_2))
