...
from cogdl_jittor.data import UnsupNeighborSamplerDataset
    ...
    def execute(self, *args):
        if isinstance(args[0], Graph):
            ...
            for i, (src_id, graph, size) in enumerate(adjs):
                ...
                if i != self.num_layers - 1:
                    x = nn.relu(x)
                    x = nn.dropout(x, p=self.dropout, training=self.is_train)
            return x
...
def train(model, dataset):
    ...
    for epoch in range(1):
        batch= UnsupNeighborSamplerDataset(
            dataset, sizes=[10,10], batch_size=16 ,shuffle=False
        )
        i=0
        for data in batch:
            i+=1
            print(i)
            # model.train()
            # target_id, n_id, adjs = data
            # x_src = dataset.data.x[n_id]
            # output = model(x_src, adjs)
...
if __name__ == "__main__":
    ...
    model = Graphsage(num_features=dataset.num_features,  num_classes=dataset.num_classes,hidden_size=[64],num_layers=2,sample_size=[10,10],aggr="mean", dropout=0.5)
