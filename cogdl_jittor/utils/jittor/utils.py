...
def set_random_seed(seed):
    jittor.misc.set_global_seed(seed, different_seed_for_mpi=True)
...
def get_norm_layer(norm: str, channels: int):
    ...
    """
    if norm == "layernorm":
        return jittor.nn.LayerNorm(channels)
    elif norm == "batchnorm":
        return jittor.nn.BatchNorm1d(channels)
    elif norm == "instancenorm":
        return jittor.nn.InstanceNorm1d(channels)
    else:
        return jittor.nn.Identity()


def cycle_index(num, shift):
    arr = jittor.arange(num) + shift
    arr[-shift:] = jittor.arange(shift)
    return arr


def batch_sum_pooling(x, batch):
    batch_size = int(jittor.max(batch)) + 1
    # batch_size = len(jittor.unique(batch))
    res = jittor.zeros(batch_size, x.size(1)).to(x.device)  
    return res.scatter_(dim=0, index=batch.unsqueeze(-1).expand_as(x), src=x,reduce='add')


def batch_mean_pooling(x, batch):
    values, counts = jittor.unique(batch, return_counts=True)
    res = jittor.zeros(len(values), x.size(1)).to(x.device)
    res = res.scatter_(dim=0, index=batch.unsqueeze(-1).expand_as(x), src=x,reduce='add')
