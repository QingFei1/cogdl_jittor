...
import jittor
import numpy as np
from cogdl_jittor.utils.graph_utils import (
    csr2coo,
    coo2csr_index,
...
from cogdl_jittor.utils import RandomWalker
...
class BaseGraph(object):
    ...
    def apply(self, func, *keys):
        ...
        for key, item in self(*keys):
            if isinstance(item, Adjacency):
                self[key] = func(item)
            if not isinstance(item, jittor.Var):
                continue
            self[key] = func(item)
        return self
    ...
    def contiguous(self, *keys):
        ...
        return self.apply(lambda x: x.clone(), *keys)
...
class Adjacency(BaseGraph):
    ...
    def add_remaining_self_loops(self):
        if self.attr is not None and len(self.attr.shape) == 1:
            ...
            self.weight = jittor.ones_like(self.row).float()
        else:
    ...
    def padding_self_loops(self):
        ...
        row, col = jittor.arange(self.num_nodes), jittor.arange(self.num_nodes
        )
        self.row = jittor.concat((self.row, row))
        self.col = jittor.concat((self.col, col))

        if self.weight is not None:
            values = jittor.zeros(self.num_nodes) + 0.01
            self.weight = jittor.concat((self.weight, values))
        if self.attr is not None:
            attr = jittor.zeros(self.num_nodes)
            self.attr = jittor.concat((self.attr, attr))
        self.row_ptr, reindex = coo2csr_index(self.row, self.col, num_nodes=self.num_nodes)
        self.row = self.row[reindex]
        self.col = self.col[reindex]
    ...
    def generate_normalization(self, norm="sym"):
        ...
        if norm == "sym":
            edge_norm = jittor.pow(degrees, -0.5)
            edge_norm[jittor.isinf(edge_norm)] = 0
            self.__out_norm__ = self.__in_norm__ = edge_norm.view(-1, 1)
        elif norm == "row":
            edge_norm = jittor.pow(degrees, -1)
            edge_norm[jittor.isinf(edge_norm)] = 0
    ...
    def _to_csr(self):
        ...
        for key in self.__attr_keys__():
            if key == "weight" and self[key] is None:
                self.weight = jittor.ones(self.row.shape[0])
            if self[key] is not None:
    ...
    def to_scipy_csr(self):
        ...
        if self.row_ptr is None:
            ...
            row_ptr = self.row_ptr.numpy()
            col_ind = self.col.numpy()
    ...
    def to_networkx(self, weighted=True):
        ...
        if weighted:
            ...
            edges = jittor.stack((row, col)).numpy().transpose()
...
class Graph(BaseGraph):
    def __init__(self, x=None, y=None, **kwargs):
        ...
        if x is not None:
            if not isinstance (x,jittor.Var):
                raise ValueError("Node features must be jittor.Var")
        self.x = x
        self.y = y
        self.grb_adj = None
    ...
    def mask2nid(self, split):
        ...
        if mask is not None:
            if mask.dtype is jittor.bool:
                return jittor.where(mask)[0]
            return mask
    ...
    def num_features(self):
        ...
        if self.x is None:
            return 0
        return 1 if len(self.x.shape) == 1 else self.x.size(1)
    ...
    def num_classes(self):
        if self.y is not None:
            return int(jittor.max(self.y) + 1) if len(self.y.shape) == 1 else self.y.shape[-1]
    ...
    def sample_adj(self, batch, size=-1, replace=True):
        if sample_adj_c is not None:
            ...
            nodes = jittor.array(_node_idx).long()

            assoc_dict = {v: i for i, v in enumerate(_node_idx)}

            col_indices = jittor.array([assoc_dict[i] for i in indices]).long()
            row_ptr = jittor.array(indptr).long()

        if row_ptr.shape[0] - 1 < nodes.shape[0]:
            padding = jittor.full((nodes.shape[0] - row_ptr.shape[0] + 1,), row_ptr[-1].item(), dtype=row_ptr.dtype)
            row_ptr = jittor.concat([row_ptr, padding])
        g = Graph(row_ptr=row_ptr, col=col_indices)
        return nodes, g

    def _sample_adj(self, batch_size, indices, indptr, size):
        if not isinstance(indices,jittor.Var):
            indices = jittor.array(indices)
        if not isinstance(indptr,jittor.Var):
            indptr = jittor.array(indptr)
        assert indptr.shape[0] - 1 == batch_size
        row_counts = (indptr[1:] - indptr[:-1]).long()
        rand = jittor.rand(batch_size, size)
        rand = rand * row_counts.view(-1, 1)
        rand = rand.long()

        rand = rand + indptr[:-1].view(-1, 1)
        edge_cols = indices[rand].view(-1)
        row_ptr = jittor.arange(0, batch_size * size + size, size)
        return row_ptr, edge_cols

    def csr_subgraph(self, node_idx, keep_order=False):
        if self._adj.row_ptr_v is None:
            self._adj._to_csr()
        if isinstance(node_idx,jittor.Var):
            node_idx = node_idx
        else:
            node_idx = jittor.array(node_idx)

        if not keep_order:
            node_idx = jittor.unique(node_idx)
        indptr, indices, nodes, edges = subgraph_c(self._adj.row_ptr, self._adj.col, node_idx)
        nodes_idx = node_idx.to(self._adj)
    ...
    def subgraph(self, node_idx, keep_order=False):
        if subgraph_c is not None:
            if isinstance(node_idx, list):
                node_idx = jittor.array(node_idx).long()
            ...
            elif isinstance(node_idx,jittor.Var):
                node_idx = node_idx.long().numpy()
            if self.__is_train__ and self._adj_train is not None:
                key = "__mx_train__"
            else:
                key = "__mx__"
            ...
            sub_g = Graph()
            sub_g.row_indptr = jittor.array(sub_adj.indptr).long()
            sub_g.col_indices = jittor.array(sub_adj.indices).long()
            sub_g.edge_weight = jittor.array(sub_adj.data)
            for key in self.__keys__():
                sub_g[key] = self[key][node_idx]
            return sub_g.to(self._adj)
    ...
    def edge_subgraph(self, edge_idx, require_idx=True):
        ...
        nodes, new_edge_index = jittor.unique(edge_index, return_inverse=True)
    ...
    def nodes(self):
        return jittor.arange(self.num_nodes)
